...
public:
  MeshBoundingBoxTest(int number_of_trials, bool verbose) {
    ...
    BOOST_FOREACH (fs::path const &p, std::make_pair(it, eod)) {
      ...
      shapes::Shape *mesh =
          shapes::createMeshFromResource("file://" + mesh_path.string());
      ...
      if (!mesh || !shapes::constructMsgFromShape(mesh, shape_msg)) {
        ROS_ERROR_STREAM_NAMED(
            "bounding box test",
            "Unable to create mesh shape message from resource " << mesh_path);
      }
      ...
      // grasp_generator_->getBoundingBoxFromMesh(mesh_msg, pose_oobb, depth,
      // width, height); // this introduces a huge
      // delay
